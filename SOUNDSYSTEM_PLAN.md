# SOUNDSYSTEM_PLAN.md â Dynamic Audio System

Companion to CLAUDE.md. Read that first â this assumes its conventions (golden
...
Code, not a finished spec â expect it to get edited as reality pushes back, same
...
- Snapshots and exposed mixer parameters **cannot both drive one parameter** (Â§1).
- `footstepSurface` is gone â the project already has `SurfaceType` (Â§10).
- A shared **`PlayerRoomTracker`** is now its own build step, because the
  "which room am I in" query is already duplicated three times (Â§2).
- **Voice budget** is now a section. It was absent, and it is the constraint most
  likely to present as a combat bug rather than an audio one (Â§5).
- **Lifecycle** is now a section: the scene RELOADS on F1 and on every depth
  change, so music dies unless it's told not to (Â§6).
- Corridors get their own `AudioProfile` slot rather than sharing the
  unauthored-type fallback (Â§3).
...
- Reverb stores explicit floats, not an `AudioReverbPreset` enum, and covers
  corridors and pits (Â§9).
- `AmbientZoneController` resolved to a **manager**, not per-room components (Â§4).

---

## 0. What already exists â don't rebuild this
...
- **Audio split by SOURCE, not situation**: established explicitly for NPCs â
  `NpcMeleeAudio` (weapon whoosh, own component and own source) vs
  `NpcCombatAudio` (effort grunts, hurt, death â body noises, and the source
...
- **Distance culling for voice slots**: `NpcMeleeAudio.cullDistance` already
  exists with the reasoning written down â inaudible sources still consume
  AudioSource slots at crowd sizes. Â§5 generalises this rather than inventing it.
- **Event hooks with no listener yet**: `ImpactAudio.OnImpact(position, loudness)`
  ...
  **ducking** (Â§8) and for a **tension nudge** (Â§7) â one event, several listeners.
- **2D vs 3D by whose sound it is**: `PlayerMeleeAudio` is 2D (your own arm), NPC
  ...
  are 3D.
- **`Surface` / `SurfaceType` / `SurfaceLibrary`** â the existing answer to "what
  is this thing made of," built for melee hit VFX/SFX. **Footsteps extend this;
  they do not get their own parallel enum.** See Â§10.
...
## 1. Mixer hierarchy (Phase 1 â foundational)

```
Master
âââ Music          (Explore / Combat / Boss â Snapshot-driven)
âââ Ambient
â   âââ Base        (always-on low drone)
â   âââ RoomType    (per-AudioProfile bed layer)
â   âââ Proximity   (point-of-interest fade-ins)
â   âââ OneShots    (drips/creaks/chitters â randomized timer pool)
âââ SFX
â   âââ Footsteps
â   âââ Combat      (NpcCombatAudio, NpcMeleeAudio, PlayerMeleeAudio, PlayerBowAudio)
â   âââ Physics     (ImpactAudio, PhysicsDoorAudio, HangingCageAudio)
â   âââ UI
âââ Voice           (NPC barks, future)
```

Every existing `AudioSource` gets an `AudioMixerGroup` assignment matching this
table â a mechanical pass, no new components, and the prerequisite for ducking
...
| music layer levels | **snapshots only** (Â§7) | `SetFloat` |
...
Exposed parameters are **decibels** (0 = unity, -80 = silence). A 0â1 UI slider
...
## 2. `PlayerRoomTracker` â one answer to "which room am I in"

**New step, and it comes before ambient, reverb and footsteps because all three
consume it.**

That query is already implemented independently in three places â
...
âââ CurrentCell   (Vector3Int)
âââ CurrentRoom   (Room, null in corridors â that is meaningful, see Â§3)
âââ CurrentPit    (PitSpec, null normally)
âââ OnRoomChanged (event â ambient crossfade, reverb blend, music baseline)
```

Refactor the existing three onto it as part of this step, don't just add a fourth.
The point is not performance (though six scans a frame is silly) â it's that the
...
(Â§12 of CLAUDE.md, the five-consumers lesson), and both reverb and ambient want to
treat it differently. Resolving it once here is cheaper than each system
rediscovering it.

---

## 3. AudioProfile â the RoomStyle of sound

Mirrors `RoomStyle` (Â§7 of CLAUDE.md) deliberately: same authoring pattern, same
fallback philosophy, same lookup point. **File name = class name** (Golden Rule
3) â `AudioProfile.cs`.

```
AudioProfile : ScriptableObject
âââ ambientBaseLayer        (AudioClip, always-on drone â usually shared)
âââ ambientRoomLayer        (AudioClip, per-type identity bed: drips/wind/crackle)
âââ oneShotPool             (AudioClip[], randomized timer pool)
âââ oneShotIntervalRange    (Vector2, seconds between one-shots)
âââ reverb                  (ReverbSettings â explicit floats, see Â§9)
âââ musicTensionBaseline    (0â1, floor for the tension signal here)
âââ voicePriority           (int, see Â§5 â ambient must never be the voice stolen)
```

Note there is **no `footstepSurface`**. See Â§10.

### Resolution, and the corridor slot

One `AudioProfile` slot on `RoomStyle` itself, not a parallel dictionary keyed by
RoomType â `RoomStyle` is already the authored per-type asset every room looks up
for its visual identity, so this is one lookup instead of two.

`RoomStyle` gains **three** audio slots, not one:

- `audioProfiles` â per `RoomType`, same shape as the wall sets.
- **`hallwayAudioProfile`** â corridors and alcoves.
- `alcoveAudioProfiles` â keyed by `AlcoveKind`, same shape as `alcoveStyles`,
  falling back to the hallway profile.

**The corridor slot is separate from the unauthored fallback on purpose.** A
corridor is not an unauthored room type â `RoomAt` returns *null* there, and a
...
type's own entry â two genuinely different concepts that look like one until
...
drone only, no room layer, no one-shots) rather than silence â matching
...
per-play-session â they aren't part of what makes `(seed, depth)` reproduce the
...
## 4. Ambient beds + one-shots â a MANAGER, not per-room components
...
1. **The voice budget (Â§5) needs a central owner.** You cannot allocate 32 voices
...
2. **Per-room components are per-regenerate garbage.** Every root a placer creates
   must be listed in `DungeonVisualizer.GeneratedRoots` or it accumulates on F1 â
...
3. **Precedent.** `TorchCullingManager` is exactly this shape â many similar
   sources, distance-gated, sliced per frame, centrally budgeted â and it is a
...
3. Only considers **active rooms** â the player's room and its immediate
...
precisely the category-audit failure Â§12 of CLAUDE.md is about â a pit opening
...
â some throne rooms want chanting and some don't, and that's authoring, not a rule.

---

## 5. Voice budget â the constraint that will present as a combat bug

**Unity's default real-voice count is 32.** Beyond that, sources are virtualised
by priority and audible distance â they silently stop being heard.

The target population is ~25 active roamers (CLAUDE.md Â§11 item 26), each capable
...
â which gets investigated as a combat bug, because that's when it happens.
...
- **Per-category allocation**, budgeted explicitly and written here once measured:
  ...
  the remainder.
- **Instrument before tuning** (Â§12): Unity's audio profiler shows real vs virtual
  voice counts live. Get a number from a real crowd fight before assigning the
  budget above â those figures are a guess and are marked as one.

---

## 6. Lifecycle â the scene reloads more than you think
...
- **Music must survive** â `DontDestroyOnLoad`, following `Hitstop`'s precedent.
...
- **Ambient must NOT survive** â it's per-room and must be rebuilt against the new
  generator.
...
- **Any static audio manager needs a `RuntimeInitializeOnLoadMethod(SubsystemRegistration)`
  ...
  otherwise survive a play-mode exit and hand out stale state â the fast-enter-
...
- **One `AudioListener` only.** The player rig runs a base camera plus a URP
  ...
  the player rig owns â check the listener doesn't get duplicated or disabled by
  the same path.
...
- **Layer 0 â Pad**: always on when music is audible at all.
- **Layer 1 â Percussion**: fades in as tension rises above the room's
  `musicTensionBaseline`.
- **Layer 2 â Melodic/combat**: fades in on real engagement.
- **Boss**: a separate cue with its own Snapshot, hard-triggered on boss-room
  entry â not a layer of the explore stack.

`TensionSend` is a single float (0â1), same "one dial, never several that can
...
what you want musically regardless. If the sight gating is ever fixed â it's a
deliberate open design call, not a bug awaiting a fix â target count becomes a
legitimate second term.

Combine terms with a weighted max plus the room's `musicTensionBaseline` floor â
...
tension crossing a threshold with hysteresis. Per Â§1, `TensionSend` itself is
...
- **Impact duck**: `ImpactAudio.OnImpact(position, loudness)` ducks Ambient and
  Music briefly, scaled by loudness. Same event Â§7 uses for tension â one hook,
...
Trivial once Â§1 and Â§7 exist; mostly Send/Receive configuration, not code.

---

## 9. Reverb per space (computed, not hand-placed)

No hand-placed Reverb Zones â room bounds are already known at generation time,
so derive reverb the way the pit and lintel systems derive geometry from existing
data rather than new authoring.

### A MIXER BUS, not a listener filter â corrected from r2
...
player's own 2D sounds â sword whoosh, bow creak, carry grunt. Those are in your
hands, not in the room.

The mixer expresses it correctly:

- A **Reverb** group under Master, chain **`Receive` â `SFX Reverb` â `Attenuation`**.
...
gets its audio processed only by effects beneath it â i.e. none â so the sends arrive
...
struct â decay time, room, room HF â matching the parameters exposed on the mixer's
...
`PlayerRoomTracker.OnRoomChanged`. Per Â§1's rule those parameters are script-driven,
so no snapshot may also animate them.

**`Room.Cells.Count` is a VOLUME, not an area** â `BuildFootprint` writes a room's
...
Â§6 of r1 covered rooms only, leaving corridors on whatever the last room set. The
two most acoustically distinctive spaces in the dungeon are exactly the ones that
weren't rooms:

- **Corridors** â tight, slapback, short decay. From `hallwayAudioProfile` (Â§3).
- **Pits** â the deep hole you can't see the bottom of. `PlayerRoomTracker.CurrentPit`
  (Â§2) is published precisely so this is a lookup rather than a rediscovery.
...
## 10. Footsteps and surfaces â extend `SurfaceType`, don't fork it
...
`SurfaceLibrary` mapping type â VFX/SFX. r1 proposed a second enum
(`Stone / Gravel / Water / Bone / Wood`) that overlaps it, adds two values and drops
two. Two enums that both answer "what is this made of" will drift â that is Â§12's
...
2. Resolve footsteps from a **short downward ray + `Surface.Of`** at each footfall â
...
3. `SurfaceLibrary` gains a footstep clip pool per type, beside its impact entries â
   one asset answering "what does this material do when struck AND when walked on."
4. Round-robin + Â±5â10% pitch jitter, matching `ImpactAudio`'s retrigger discipline
   for the same "same sound N times in a row" problem.

**Covers `NpcFootsteps` as well as `PlayerFootsteps`** â they already both exist, and
...
## 11. Occlusion (lowest priority â and NOT via the room graph)
...
while passing sound freely down a corridor that should attenuate it â wrong in both
directions.
...
Mark done inline as this progresses, same convention as CLAUDE.md Â§11.

1. [x] **Mixer asset + group hierarchy** (Â§1). Route every existing `AudioSource`
   to its group. No new behaviour â pure plumbing; verify nothing changed
   volume-wise. Establish the one-driver-per-parameter table before anything reads
   it.
2. [x] **`PlayerRoomTracker`** (Â§2) â *new step.* Extract the room lookup and
   ...
   nothing changing.
3. [x] **`AudioProfile`** (Â§3) + `RoomStyle` slots (per-type, hallway, alcove) +
   `DefaultAudioProfile`.
4. [x] **`AmbientDirector`** (Â§4) â base + room layers first; one-shot pool and
   proximity points as a follow-up once the crossfade is stable. Root registered in
   `GeneratedRoots` if it creates one.
   **Built as a manager watching the RESOLVED PROFILE, not `OnRoomChanged`** â
...
5. [x] **Voice budget instrumentation** (Â§5) â measure a real crowd fight, then set
   the per-category allocation. Do this BEFORE music stems, so the budget is known
   before more sources are added to it.
   **Measured: peak 14 voices, 0 stolen, at 50 NPCs mid-fight â double the target
   ...
   `playOnAwake` with a null clip (see Â§5 and CLAUDE.md Â§10b). Culling and priorities
   were built first against a plausible misreading of the numbers, and were not the
   fix â they are kept on their own merits.
6. [x] **Reverb by space** (Â§9) **+ footstep surfaces** (Â§10) â both consume #2;
   do them together.
   ...
   dB** â the r2 draft's values were authored as dB and would have left every space
   fully wet; see Â§9. Footsteps EXTENDED `SurfaceType` rather than forking it, as
   Â§10 required. Field-tuned: the room population is bimodal (closets ~2 cells,
   ordinary rooms 60-150), so the size blend spans 10..165 and closets clamp.
7. [ ] **Adaptive music stems + Snapshots + `TensionSend`** (Â§7) â scaffold with
   placeholder loops; needs the `DontDestroyOnLoad` decision from Â§6 in place.
8. [ ] **Ducking** (Â§8).
9. [x] **Occlusion** (§11) — lowest priority, reworked premise.
   PULLED FORWARD ahead of #7/#8: it depends on neither, and they are blocked on the stems
   decision. The plan's gate ("only worth building once bleed-through is a NOTICED problem")
   had been met by the torch crackle and NPC audio landing. Building it immediately exposed
   three bugs that had nothing to do with occlusion — ambient one-shots placed inside walls,
   a crossfade that could not be interrupted, and impacts that would occlude themselves at
   their own contact point — two of which were shipping silently. Worth remembering as an
   argument for building "can the player actually perceive this" checks earlier.
...
- ~~Per-room component or manager?~~ **Manager** (Â§4). The voice budget needs a
...
- **Music stems: authored or sourced?** Affects only whether #7 starts with real
  ...
  doesn't block anything â but it's the longest lead-time item, so decide early.
...
- **Should the glow of a stained-glass window imply an ambient source?** A lit window
  ...
  wall prefab via the kit-socket system (CLAUDE.md Â§8) rather than anything new â
