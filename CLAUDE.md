...
**UNITY'S MESH LOD DOES NOT WORK ON THE INSTANCED PATH. DO NOT TRY IT AGAIN — the docs say it
does.** Mesh LOD (Unity 6.2+) bakes reduced levels into a mesh's index buffer, generated by a
tickbox on the model importer with no DCC work, which makes it look like exactly the lever a
114M-triangle GPU-bound frame wants. Its limitations page lists `Graphics.RenderMeshInstanced`
among APIs that get no automatic screen-size selection and must set `RenderParams.forceMeshLod`
instead — implying it works if you do. **It does not.** The same page separately lists "GPU
instancing" among systems that always use LOD0, and that is the line that governs.
**THE PROOF IS THE OUT-OF-RANGE CASE, which is what makes this worth writing down**: an
out-of-range `forceMeshLod` makes geometry VANISH, so the field is demonstrably read and
range-checked — it is simply not honoured for selection. Verified against `Statue_King`, whose
LOD 7 is heavily decimated in the Inspector and renders identically to LOD 0 in game, with no
GPU-time change forcing maximum reduction across ~50% of the dungeon's geometry.
**THE TRIANGLE COUNTER CANNOT ADJUDICATE THIS**, and neither can the eye. The Stats panel warns
"Triangle and vertex counts might not reflect the actual values due to indirect draw calls", so
a flat count is not evidence either way; and a GOOD LOD is supposed to look the same, so "the
wall did not visibly coarsen" is equally worthless. The test that works is the one above:
a mesh whose reduction you have CONFIRMED in the Inspector, forced, and compared. Note also that
Mesh LOD reduces the INDEX buffer, so `Mesh.vertexCount` never moves however many levels exist.
Skinned meshes are separately pointless here — the generator ignores skin weights, and a
`SkinnedMeshRenderer` always deforms LOD0 whatever level draws.
**The remaining route to fewer triangles is DCC-side reduction**, aimed by the geometry cost
report below. `LODGroup` is the other untried option and is a different mechanism (real
GameObjects, per-level renderers) that this project's instanced path also has no use for.

**"WHERE ARE MY TRIANGLES" IS A QUESTION ONLY THE RENDERER CAN ANSWER —
`InstancedDungeonRenderer.LogGeometryCostReport` (context menu, also forwarded from
`DungeonVisualizer`).** Cost is per-instance detail MULTIPLIED BY how often the generator places
it, and no amount of looking at the Project window reveals that. First run, depth 20:

| verts x instances | mesh | per-instance | count |
|---:|---|---:|---:|
| **18.5 M** | `Ceiling_Plain` | 24,749 | 747 |
| 7.1 M | `Wall_Plain` | 6,242 | 1130 |
| **6.4 M** | `Ceiling_Plain .001` | 24,749 | 259 |
| 4.3 M | `CobblestoneTile` | 5,738 | 753 |
| 3.3 M | `CrawlSpace_Pipe_Tee` | 77,541 | 42 |

65.9M verts total, and **a CEILING tile is 38% of it across its two variants** — the surface the
player looks at least, at four times the vertex cost of a wall. Nobody suspected it; the report
found it in one click. **It also surfaces DUPLICATE MESHES**: two entries with an identical
vertex count and a Blender `.001` suffix are one mesh imported twice, and since `BatchKey`
includes the mesh they can NEVER merge — that is two batches drawing the same thing, worth
consolidating whatever else happens.
...
28b. ⏳ **Triangle budget** — the lever the re-measurement actually points at.
    **Unity's Mesh LOD was investigated and DOES NOT WORK on the instanced path** (§5 — read that
    before reaching for it; the docs say it does, and the negative result is expensive to
    re-derive). So the route is DCC-side reduction, aimed by
    `LogGeometryCostReport`. Targets, worst first: **`Ceiling_Plain` at 24,749 verts x 1006
    instances across two variants = 38% of the dungeon's geometry**, on the surface players look
    at least; `CobblestoneTile`; the crawlway pipes at 77.5k verts per tee, seen from inside a
    1.5m bore while crouched. Also check whether `Ceiling_Plain` / `Ceiling_Plain .001` are one
    mesh imported twice — duplicates can never share a batch. NB décor INSTANCE count is not the
    problem — 705 extra props measured free (§5) — so this is about per-mesh cost, not prop
    density. `LODGroup` remains untried and is a different mechanism from Mesh LOD.
