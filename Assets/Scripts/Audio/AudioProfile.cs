...
namespace DungeonGen
{
    ...
    public class AudioProfile : ScriptableObject
    {
        ...
        [Tooltip("Where in the room's HEIGHT a one-shot can happen, as a fraction of the OPEN COLUMN above the chosen cell (0 = floor, 1 = ceiling).\n\nA fraction rather than metres so it means the same thing everywhere: a 3m corridor and a two-storey hall both use their whole height from one setting, where an authored metre range would either clip the hall or float in the corridor.\n\nEverything at floor level reads flat and, worse, LOCATES badly — the ear places height poorly, so a drip that comes from above is mostly telling you the space has a ceiling. Keep the top below 1 or sounds sit inside the ceiling slab.")]
        public Vector2 oneShotHeightRange = new Vector2(0.05f, 0.85f);
